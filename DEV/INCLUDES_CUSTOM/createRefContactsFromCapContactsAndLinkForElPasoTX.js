function createRefContactsFromCapContactsAndLinkForElPasoTX(pCapId, contactTypeArray, ignoreAttributeArray, replaceCapContact, overwriteRefContact, refContactExists)
	{
	   //-- Custom, check email address.  If DUMMY@elpasotexas.gov, do not create/link ref contact.
	...
	for (var i in c)
	   ...
	   // Check for DUMMY email address
	   if(con.getEmail() == "DUMMY@elpasotexas.gov")
		{ 
			logDebug("Skipping logic for this contact, because it has a DUMMY email address.  Do not create or link Ref Contact");  
			continue; 
		}
