function createPublicUserFromContactForElPasoTX()   // optional: Contact Type, default Applicant
{
	// --  12/17/2013 - Check for Contact Type Flag before doing a compare, otherwise an error is raised for null using .equals
        // --  02/12/2014 - Ignore APPLICANT with email of DUMMY@elpasotexas.gov
	...
	//CUSTOM - Check for Dummy email address.  Need to exclude
	//There should be an email at this point based on check above.
	if (contact.getEmail() == "DUMMY@elpasotexas.gov")
    { logDebug("Couldn't create public user for " + contactType + ", this contact has a DUMMY email address"); return false; }

	var contactTypeFlag = contact.getPeople().getContactTypeFlag();
	if(contactTypeFlag)
